...
from floorplan_ai.geometry import extract_planes, write_ply
...
def build_model(capture_inputs, result:ReconstructionResult, output_dir:Path, source_type:str, timestamps:dict[str,float]|None=None)->CanonicalWorldModel:
    ...
    for raw in result.camera_models:
        ...
        cameras.append(Camera(camera_id=cid,capture_id=default_capture,focal_length=focal,principal_point=pp,distortion_coefficients=dist,resolution=(raw.width,raw.height),prior_source='reconstruction' if focal else 'reconstruction (degraded model: '+raw.model+')'))
    poses=[]; pose_by_name={}
