...
from .common import build_model,stable_id
...
def reconstruct_video(capture:CaptureInput,output_dir:Path,config:VideoFrontendConfig|None=None):
 ...
 model=build_model(inputs,result,output_dir,'video',config.plane); poses={p.image_name:p.image_id for p in result.poses}; trajectory=[{'frame_id':f.frame_id,'timestamp_seconds':f.timestamp_seconds,'source_capture_id':str(model.captures[i].capture_id),'image_path':str(f.path),'pose_id':str(stable_id('pose/'+str(poses[Path(f.path).name]))) if Path(f.path).name in poses else None} for i,f in enumerate(selected)]; (output_dir/'reconstruction'/'trajectory.json').write_text(json.dumps(trajectory,indent=2,sort_keys=True)); diag=json.loads((output_dir/'diagnostics.json').read_text()); diag.update({'frames_decoded':len(frames),'frames_selected':len(selected),'frames_rejected':len(rejected),'trajectory_pose_count':len(model.poses)}); (output_dir/'diagnostics.json').write_text(json.dumps(diag,indent=2,sort_keys=True)); return model
