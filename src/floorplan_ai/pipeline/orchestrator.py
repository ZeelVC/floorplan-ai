...
def run_reconstruction(
    ...
    if config.depth_enabled and not config.depth_model_path.exists():
        raise RuntimeError(
            f"metric_depth: Depth Pro checkpoint missing: {config.depth_model_path}"
        )

    depth_config = (
        MetricDepthConfig(DepthProEstimator(config.depth_model_path, config.depth_device))
        if config.depth_enabled
        else None
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    mode = detect_input(input_path)

    if mode == "video":
        model = reconstruct_video(
            CaptureInput(
                capture_id="video",
                source_type="video",
        ...
        for group in photo_groups(input_path):
            ...
            inputs = tuple(
                CaptureInput(
                    capture_id=f"{group.capture_id}-{i}",
                    source_type="photo",
                    file=p.name,
                    resolved_file=p,
                    metadata={"room_group_id": group.room_group_id}
                    if group.room_group_id
                    else {},
                )
            ...
            local_models.append(
                infer_structure(
                    reconstruct_photos(
                        inputs,
                        local_output,
                        PhotoFrontendConfig(metric_depth=depth_config),
                    ),
                    StructuralInferenceConfig(artifact_root=local_output),
                )
            )
    ...
    export_json(model, output_dir / "floorplan.json")
    export_svg(model, output_dir / "floorplan.svg")
    export_dxf(model, output_dir / "floorplan.dxf")

    diagnostics = (
        json.loads((output_dir / "diagnostics.json").read_text())
        if (output_dir / "diagnostics.json").exists()
        else {}
    )
    diagnostics["stitching_components"] = stitching_components
    diagnostics.update(
        {
            "drift_correction_enabled": not config.disable_drift_correction,
    ...
    (output_dir / "provenance.json").write_text(
        model.provenance.model_dump_json(indent=2) if model.provenance else "{}"
    )
