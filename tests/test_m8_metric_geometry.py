...
def test_photo_metric_depth_flows_to_structure_and_opening_measurement(tmp_path, monkeypatch):
    ...
    def reconstruction(*args, **kwargs):
        return ReconstructionResult(
            ...
            camera_models=(ReconstructionCamera(camera_id=1, model="SIMPLE_PINHOLE", width=100, height=100, params=(50.0, 50.0, 50.0)),),
            poses=(ReconstructionPose(image_id=1, image_name="room.jpg", camera_id=1, qvec=(1.0, 0.0, 0.0, 0.0), tvec=(0.0, 0.0, 0.0)),),
            images=(ReconstructionImage(image_id=1, name="room.jpg", camera_id=1),),
            points=tuple(ReconstructionPoint(point_id=i, xyz=point) for i, point in enumerate(sparse_xyz)),
    ...
    metric_capture = CaptureInput(capture_id="room", source_type="photo", file=image_path.name, resolved_file=image_path, metadata={"focal_length": 4.0})
    model = reconstruct_photos(
        (metric_capture,),
        tmp_path / "result",
        PhotoFrontendConfig(reconstruction=Backend(), metric_depth=common.MetricDepthConfig(estimator=Depth(), depth_stride=10, maximum_depth_points=500)),
    )
    ...
    fused_path = tmp_path / "result" / model.geometries[0].vertex_buffer_reference
    assert fused_path.is_file()
    fused = np.loadtxt(fused_path)
    ...
    def fake_openings(points, walls, *, floor_height, ceiling_height, config=None):
        ...
        wall = next(item for item in walls if item.start_point_2d[1] == pytest.approx(-2.0))
        return (Opening(parent_wall_id=wall.wall_id, opening_type=OpeningType.DOOR, offset_along_wall=1.0, width=0.9, height=2.1, sill_height=0.0, connected_room_ids=wall.room_ids, uncertainty=Uncertainty(distribution_type="synthetic", confidence_bounds=(0.0, 0.01)), provenance=Provenance(generating_pipeline_stage="opening_inference")),)

    monkeypatch.setattr(structural, "infer_openings", fake_openings)
    structured = infer_structure(model, StructuralInferenceConfig(artifact_root=tmp_path / "result"))
    ...
    opening_measurements = [item for item in measurements if item.metric_type == "OPENING_WIDTH" and item.target_entity_id == structured.openings[0].opening_id]
